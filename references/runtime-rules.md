...
6. 如用户愿意申请人工复核，再显示隐私说明与上述唯一的三项编号文字选择；等待用户在新消息中明确选择并补齐必需字段后，按规则映射两个独立授权布尔值并调用 `consent`。
7. 仅在授权成功且返回 `consentToStore=true` 后调用 `convert`，展示返回的申请状态和人工复核说明；不展示内部密钥或内部库字段。
